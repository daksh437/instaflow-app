...
import 'package:insta_flow/screens/ai_caption_screen.dart';
...
import 'package:insta_flow/screens/caption_generator_screen.dart';
...
import 'package:insta_flow/screens/coming_soon_stats_screen.dart';
import 'package:insta_flow/screens/your_stats_screen.dart';
import 'package:insta_flow/screens/instagram_automation/schedule_post_screen.dart';
...
void main() {
  runZonedGuarded<Future<void>>(() async {
    ...
    try {
      ...
      if (kDebugMode) debugPrint('STARTUP: Firebase init timeout/error: $e');
    ...
    runApp(
      MultiProvider(
        ...
        child: const InstaFlowApp(),
      ),
    );
    if (kDebugMode) debugPrint('STARTUP: runApp done');
  }, (error, stack) {
    if (kDebugMode) debugPrint('STARTUP: zone error $error $stack');
    FirebaseCrashlytics.instance.recordError(error, stack ?? StackTrace.current, fatal: false);
  });
}
      ...
      child: ForceUpdateGate(
        child: StreamBuilder<User?>(
          ...
          builder: (context, snapshot) {
            ...
            final child = user != null ? const HomeScreen() : const LoginScreen();
            return SessionGuardGate(child: child, isLoggedIn: user != null);
          },
        ),
      ),
    );
...
class _SessionGuardGateState extends State<SessionGuardGate> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && widget.isLoggedIn) _runSessionCheck();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/stats-full': (context) => const StatsScreen(), // Full stats (when ready)
        '/instagram-stats': (context) => const InstagramStatsScreen(),
        '/ai-tools': (context) => const AIToolsScreen(),
        '/captions': (context) => const CaptionGeneratorScreen(),
        ...
        '/ai_caption': (context) => const AICaptionScreen(),
        ...
        '/schedule-post': (context) => const SchedulePostScreen(),
